// L'admin du praticien · boucle admin (spec §4).
// Une seule page, deux blocs : les réservations, les créneaux.
// Accès par clé (décision tracée dans docs/boucles/ADMIN-PLAN.md).
...
export default async function Admin({
  ...
  const creneaux = getTousCreneauxFuturs();
    ...
    <main className="mx-auto max-w-2xl p-6">
      ...
      {reservations.length === 0 ? (
        ...
        <ul className="mb-8 flex flex-col gap-3">
          {reservations.map((r) => {
            const creneau = getCreneau(r.creneau_id);
            return (
      ...
      <ul className="flex flex-col gap-1.5">
        {creneaux.map((c) => (
          ...
          >
            ...
            {c.reserve ? (
              <span className="text-xs font-semibold text-sauge">réservé</span>
            ) : (
